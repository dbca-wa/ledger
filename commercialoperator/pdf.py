...
def create_confirmation(confirmation_buffer, booking, park_bookings, cols_var):
    ...
    to_date = ""
    parks = []
    for i, pk in enumerate(park_bookings):
        if pk.campsite.mooringarea not in parks:
            parks.append(pk.name)
        arrival = pk.arrival.strftime('%d/%m/%Y')
        ...
        if from_date == "":
            from_date = start
        if i == len(park_bookings)-1:
            to_date = end_str
        else:
            next_dt = park_bookings[i+1].from_dt
            ...
            if (end + timedelta(minutes=1)) == next_dt and mb.campsite.mooringarea.name == park_bookings[i+1].campsite.mooringarea.name:
                #Go to next booking
                to_date = ""
            else:
                to_date = end_str
...
#    table_data.append([Paragraph('Number of guests', styles['BoldLeft']), Paragraph(booking.stay_guests, styles['Left'])])
    ...
    for park in parks:
        if parkg.additional_info:
            additional_info += park.name + ": <br/>"
            additional_info += park.additional_info + "<br/>"
...
def create_admissions_confirmation(confirmation_buffer, admissionsBooking, cols_var):
