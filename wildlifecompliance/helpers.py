...
def is_wildlifecompliance_admin(request):
    return return request.user.is_authenticated() and is_model_backend(request) and in_dbca_domain(request) and ((belongs_to(request.user, 'Wildlife Compliance Admin')) or request.user.is_superuser)
