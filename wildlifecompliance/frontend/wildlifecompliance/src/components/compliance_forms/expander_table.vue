...
const ExpanderTable = {
  ...
  methods: {
    ...mapActions(["removeFormInstance", "setFormValue"]),
    ...
    removeTable: function(tableId) {
      ...
      this.removeFormInstance(this.getInstanceName(tableId));
    ...
    addNewTable: function(params = {}) {
      ...
      this.updateVisibleTables(this.existingTables);
    },
  ...
  computed: {
    ...
    lastTableId: function() {
      ...
      let lastId = 0;
      this.existingTables.map(
        tableId =>
          tableId[tableId.length - 1] > lastId &&
          (lastId = tableId[tableId.length - 1])
      );
