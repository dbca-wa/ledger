...
class Command(BaseCommand):
    help = 'Import identification of EmailUser from temporary folder'
            ...
            for f in entries_path.iterdir():
                if f.is_file():
                    ...
                    try:
                        ...
                        if u_id and type_name.startswith('identification'):
                            usr=EmailUser.objects.get(id=u_id)
                            ts=calendar.timegm(time.gmtime())
                            if f.is_file() and not usr.identification2:
                                rb=f.read_bytes()
                                ...
                                p.save()
                                usr.identification2=p
                                usr.save()
                                updates.append(usr.id)
                    except Exception as e:
                        err_msg = 'Error copying Identification doc for EmailUser {}'.format(u_id)
