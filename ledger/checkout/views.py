...
class IndexView(CoreIndexView):
    ...
    def __validate_card_method(self, method):
        ''' Validate if the card method is payment or capture
        '''
        if method in ['capture','payment']:
            # 
    ...
    def handle_payment(self, order_number, total, **kwargs):
        ...
        if method == 'card':
            try:
                ...
                resp = bpoint_facade.post_transaction('payment','internet','single','checkout',order_number,invoice.reference, total.incl_tax,kwargs['bankcard'])
